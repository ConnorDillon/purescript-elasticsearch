...
import Database.ElasticSearch (Client, alias, aliases, bool, bulk, bulkCreate, bulkDelete, bulkIndex, bulkUpdate, client, create, createIndex, createIndexBody, delete, deleteIndex, doc, get, index, long, mapping, mappings, match, matches, search, searchBody, settings, toObject, update, waitFor)
...
testCreateIndexIndexSearchDeleteIndex c idx = test "createIndex/index/search/deleteIndex" do
  ...
  where
    body = createIndexBody 
      { aliases: aliases {baz: alias {is_hidden: true , is_write_index: true , routing: "shard-1"}}
      , mappings: mappings {bar: mapping {type: long}}
...
testCreateUpdateGetDelete c idx = test "create/update/get/delete" do
  ...
  _ <- create c {index: idx, refresh: waitFor, id: id, body: toObject source} {}
  ...
  pure $
    uorToMaybe gr1.body._source == Just (toObject source)
    && uorToMaybe gr2.body._source == Just (union (toObject source) (toObject patch))
...
testBulk c idx = test "bulk" do
  _ <- createIndex c {index: idx} {}
  br1 <- runBulk $ bulkCreate {_id: "foo1"} {bar: 1}
                    <> bulkIndex {_id: "foo2"} {baz: 2}
  br2 <- runBulk $ bulkUpdate {_id: "foo1"} {bar: 3}
                    <> bulkUpdate {_id: "foo2"} {baz: 4}
